...
public class RepositoryForStudentJDBC implements RepositoryForStudentsInterface {
    ...
    public List<Student> findAll() {
        ...
        try(Connection connection = DataSource.getConnection();
            ...
            PreparedStatement ps2 = connection.prepareStatement("select * from rating;");
            ResultSet rs2 = ps2.executeQuery();
            while (rs2.next()) {
                int id = rs2.getInt("student_id");
                Subject subject = Subject.getSubjectByString(rs2.getString("subject"));
    ...
    public Student save(Student student) {
        ...
        try(Connection connection = DataSource.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement("insert into student" +
                    " (id, login, password, name, age) values (?, ?, ?, ?, ?);")
        ) {
        ...
        for (Map.Entry<Subject, Integer> entry: student.getRatings().entrySet()) {
            try (Connection connection = DataSource.getConnection();
                 PreparedStatement ps2 = connection.prepareStatement("insert into rating (student_id, subject, rating) " +
                         "values (?, ?, ?);")
            ) {
...
//            PreparedStatement preparedStatement = connection.prepareStatement("select * from student where login = ?");
